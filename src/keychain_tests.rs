...
mod tests {
    ...
    fn test_composio_key_persistence_cycle() {
        ...
        let save_result = keychain_ffi::set_generic_password_with_biometric_protection(&key_name, SECRET_VALUE)
            .or_else(|e| {
        ...
        assert!(save_result.is_ok(), "Failed to save secret: {:?}", save_result.err());
