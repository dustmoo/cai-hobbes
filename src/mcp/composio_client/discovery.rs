...
pub async fn list_connected_toolkits(client: &ComposioClient) -> Result<Vec<ToolkitInfo>, String> {
    ...
    let mut toolkit_map: std::collections::HashMap<String, usize> =
...
pub async fn list_all_toolkits(
    ...
    tracing::trace!(
        "Fetched {} toolkits (page {} of {}, next_cursor: {:?})",
        parsed.items.len(),
        current_page,
        total_pages,
        parsed.next_cursor
...
pub async fn get_toolkit_tools(
    ...
    tracing::info!(
        "Found {} tools for toolkit '{}' (prefix: {})",
        filtered_tools.len(),
        toolkit_slug,
        prefix
    );
...
pub async fn get_toolkit_tools_detailed(
    ...
    // Filter tools by toolkit prefix convention: TOOLKIT_SLUG_UPPERCASE_*
    let prefix = format!("{}_", toolkit_slug.to_uppercase());
    let filtered_tools: Vec<(String, Option<String>)> = all_tools
        .into_iter()
        .filter(|tool| tool.starts_with(&prefix))
...
pub async fn list_tools_filtered(
    ...
    let filtered: Vec<ComposioTool> = all_tools
        .into_iter()
        .filter(|tool| {
            let tool_toolkit = tool
                .toolkit
                .as_ref()
                .map(|tk| &tk.slug)
            ...
            if let Some(tk) = tool_toolkit {
                return filter_slugs
                    .iter()
                    .any(|slug| tk.eq_ignore_ascii_case(slug));
            }
...
pub async fn list_tools_for_session(
    ...
    tracing::trace!(
        "Total tools for session: {} (including {} meta-tools)",
        tools.len(),
        2
    );
                ...
                if json_value.is_object() {
                    ...
                    if json_obj.get("jsonrpc").is_some() {
                        if let Ok(rpc_response) = serde_json::from_value::<
                            JsonRpcResponse<Vec<ComposioTool>>,
                        >(json_obj.clone())
                        {
                    ...
                    if let Some(result) = json_obj.get("result") {
                        if let Some(tools) = result.get("tools").and_then(|t| {
                            serde_json::from_value::<Vec<ComposioTool>>(t.clone()).ok()
                        }) {
                            return Ok(tools);
                        }
                    }
                    if let Some(tools) = json_obj
                        .get("tools")
                        .and_then(|t| serde_json::from_value::<Vec<ComposioTool>>(t.clone()).ok())
                    {
                        return Ok(tools);
                    }
