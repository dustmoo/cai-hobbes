...
impl AuthenticatedSseClient {
    ...
    async fn check_response(
        ...
        response
            .error_for_status()
            .map_err(AuthenticatedClientError::Reqwest)
    }
}
...
impl SseClient for AuthenticatedSseClient {
    ...
    > {
        ...
        request_builder = request_builder.header(
            ACCEPT,
            format!("application/json, {}", EVENT_STREAM_MIME_TYPE),
        );
        ...
        if uri_str.contains("composio.dev") && content_type.starts_with("application/json") {
            ...
            tracing::error!(
                "Expected SSE stream from Composio but got JSON: {}",
                body_str
            );
