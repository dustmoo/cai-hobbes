...
pub fn CommentModal(
    ...
    rsx! {
        div {
            ...
            div {
                class: "bg-card rounded-lg p-6 max-w-2xl w-full mx-4 max-h-[80vh] overflow-y-auto",
                tabindex: "0",
                ...
                div {
                    class: "mb-4",
                    label {
                        class: "block text-sm font-medium text-fg-muted mb-2",
                        "Message Content"
                    }
                    div {
                        class: "bg-app p-3 rounded-lg text-sm text-fg-muted max-h-40 overflow-y-auto",
                        "{message_text}"
                    }
                }
                ...
                // Add comment button
                button {
                    class: "w-full mb-4 px-4 py-2 bg-btn-primary hover:bg-btn-primary-hover text-fg rounded-md transition-colors",
                    onclick: move |_| {
                ...
                if !comments().is_empty() {
                    div {
                        class: "mb-4",
                        h3 {
                            class: "text-sm font-medium text-fg-muted mb-2",
                            "Comments ({comments().len()})"
                        }
                        for comment in comments().iter() {
                            div {
                                key: "{comment.id}",
                                class: "bg-app p-3 rounded-lg mb-2",
                                div {
                ...
                div {
                    ...
                    button {
                        class: "px-4 py-2 bg-input hover:bg-input text-fg rounded-md transition-colors",
                        onclick: move |_| {
                            focus_context.set(FocusContext::ChatInput);
                            on_close.call(());
                        },
                    ...
                    button {
                        class: "px-4 py-2 bg-btn-primary hover:bg-btn-primary-hover text-fg rounded-md transition-colors",
                        onclick: move |_| {
                            on_save.call(comments());
                            focus_context.set(FocusContext::ChatInput);
                            on_close.call(());
