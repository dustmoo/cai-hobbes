...
-   **`GlamaClient`**: A dedicated client for the public Glama MCP registry API (`https://glama.ai/api/mcp/v1`, no API key). It handles searching/browsing servers with attribute filters (local/remote/hybrid hosting, official) and cursor pagination, parses each server's env-var JSON schema, and derives local install commands from repository manifests (`package.json` → `npx`, `pyproject.toml` → `uvx`). Locally installed registry servers run under an OS sandbox by default (`src/mcp/sandbox.rs`) with per-server allowed directories and a network toggle, and start with per-tool-call permission prompts.
-   **MCP Sandbox** (`src/mcp/sandbox.rs`): OS-level confinement for locally launched registry servers, applied by rewriting the launch command (`wrap_command`). All platforms share the same philosophy: broad read access **minus** sensitive user data (the toolchain needs to read system libraries, so a read allowlist is impractical), a strict write allowlist (temp + npm/uv caches + user-granted dirs), and a per-server network toggle.
    -   **macOS**: `sandbox-exec` with a generated Seatbelt profile. The read denylist covers Documents/Desktop/Pictures/Downloads, `~/Library/Application Support` (including Hobbes' own `sessions.json`/`settings.json`), Messages, Mail, Safari, Cookies, HTTPStorages, Containers, CloudStorage, Keychains, `~/.ssh`, `~/.aws`, `~/.gnupg`, `~/.config`, `~/.kube`, `~/.docker`, `.netrc` and shell history. User-granted `allowed_paths` re-enable access (later Seatbelt rules win).
    -   **Linux**: `bwrap` (bubblewrap) when installed — read-only root bind, tmpfs masks over the sensitive dirs (plus `~/.mozilla` and keyring stores), rw binds for caches and granted paths, `--unshare-net` when network is off, `--die-with-parent`.
    -   **Windows**: `hobbes-sandbox.exe` (`apps/sandbox_shim`), an AppContainer broker shipped next to `hobbes.exe`. Stable Rust cannot attach AppContainer capabilities through `tokio::Command`, so the broker does `CreateProcessW` with a container SID (deny-by-default on the whole user profile), grants the SID access to npm/uv caches, temp and `--allow` paths via ACLs, adds internet capabilities only with `--net`, and parents the child under a kill-on-close job object.
    -   `sandbox_available()` gates the default: registry installs (`source: "glama"`) sandbox by default only when the platform mechanism actually exists; an *explicit* `sandbox: true` with no mechanism is a launch error (recorded in `failed_servers`), never a silent unsandboxed run.
    -   **Env hygiene**: registry-installed servers launch with `env_clear()` — they receive only their configured env, keychain-resolved `secret_env` values, and the critical vars (HOME/USER/SHELL/TMPDIR/PATH), never Hobbes' full process environment (which can hold dotenv-loaded API keys). Env vars with credential-looking names are stored in the OS keychain (`mcp_env_<server>__<VAR>`) rather than plaintext `mcp_servers.json`, and are deleted when the server is removed.
