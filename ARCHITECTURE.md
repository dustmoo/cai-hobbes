...
-   **`McpManager`**: A central service responsible for managing the lifecycle of all MCP servers. It manages both local child processes (standard MCP) and remote **Composio Integration (custom MCP)**. It synchronizes tool lists and handles the "Tool Router" logic, allowing for a hybrid of "Force Loaded" tools (always available) and "On-Demand" lookup.
...
-   **`PromptBuilder`**: A utility that reads the `active_context` and `tool_call_history` from the current `Session`. It assembles the structured prompt and performs **Type-Safe Schema Conversion** (`mcp_tool_to_gemini`). This translates generic JSON Schemas into Gemini-compatible `FunctionDeclarations` (handling `oneOf` flattening, type enforcement, etc.), enabling "Direct" tool use without hallucinated XML.
